...
export function ExplorerPage() {
  ...
  async function loadPage(cursor: string | null, reset = false) {
    ...
    try {
      ...
      const data = await getGraph(params); setNodes(data.nodes); setEdges(data.edges); setRequestCursor(cursor); setNextCursor(data.nextCursor); setSelected(null); setTrail([]); setError('');
  ...
  async function isolateNode(id: string) {
    setLoading(true);
    try { const clicked = nodes.find(node => node.id === id); const data = await getNeighbors(id); const focus = data.nodes.find(node => node.id === id) ?? clicked ?? data.nodes[0] ?? null; setNodes(data.nodes); setEdges(data.edges); setSelected(focus); if (focus) setTrail(value => [...value.filter(item => item.id !== focus.id), { id: focus.id, name: focus.name, kind: getNodeKind(focus) }].slice(-5)); setError(''); }
  ...
  const incoming = selected ? edges.filter(edge => edge.target === selected.id).length : 0;
  const outgoing = selected ? edges.filter(edge => edge.source === selected.id).length : 0;
