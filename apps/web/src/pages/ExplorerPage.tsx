...
export function ExplorerPage() {
  ...
  const merge = (incoming: { nodes: GraphNode[]; edges: GraphEdge[] }) => { setNodes(v => [...new Map([...v, ...incoming.nodes].map(x => [x.id, x])).values()]); setEdges(v => [...new Map([...v, ...incoming.edges].map(x => [x.id, x])).values()]); };
