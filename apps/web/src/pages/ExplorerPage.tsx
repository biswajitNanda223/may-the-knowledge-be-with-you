...
export function ExplorerPage() {
  ...
  const load = useCallback(async (reset = false) => { try { const p = new URLSearchParams({ limit: '25' }); if (!reset && cursor) p.set('cursor', cursor); if (search) p.set('search', search); const data = await getGraph(p); if (reset) { setNodes(data.nodes); setEdges(data.edges); } else merge(data); setCursor(data.nextCursor); setError(''); } catch (e) { setError(e instanceof Error ? e.message : 'Failed'); } }, [cursor, search]);
