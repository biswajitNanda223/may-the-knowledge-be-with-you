...
function rankStarts(question: string, nodes: GraphNode[], targets: string[]) {
  ...
  const score = (node: GraphNode) => {
    const text =
      `${node.id} ${node.name} ${Object.values(node.properties).join(" ")}`.toLowerCase();
    return terms.reduce(
...
function buildAdjacency(edges: GraphEdge[]) {
  ...
  const add = (from: string, step: Step) =>
    adjacency.set(from, [...(adjacency.get(from) ?? []), step]);
...
function toRoute(path: Path, nodeById: Map<string, GraphNode>): EvidenceRoute {
  ...
  path.nodes.forEach((id, index) => {
    route.elements.push(id);
    route.labels.push(nodeById.get(id)?.name ?? id);
    const edge = path.edges[index];
    if (edge) {
      route.elements.push(edge.id);
      route.labels.push(edge.type.replaceAll("_", " ").toLowerCase());
    }
  });
  return route;
}

export function buildEvidenceRoutes(
  question: string,
  nodes: GraphNode[],
  edges: GraphEdge[],
  targets: string[],
  ...
  for (const target of targets.slice(0, 4)) {
    const candidates = starts
      .map((start) => shortestPath(start.id, target, adjacency))
      .filter((path): path is Path => Boolean(path));
    const best = candidates.sort(
      (left, right) => left.edges.length - right.edges.length,
    )[0];
    if (best) routes.push(toRoute(best, nodeById));
  }
  if (!routes.length && starts[0])
    routes.push({ elements: [starts[0].id], labels: [starts[0].name] });
  return routes;
}
