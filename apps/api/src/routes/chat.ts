...
export const chatRoutes: FastifyPluginAsync = async app => {
  ...
  app.post('/', async (req, reply) => {
    ...
    } catch (error) { if(traceAdk) adkTelemetry.fail(traceId,error instanceof Error?error.name:'UnknownError'); send('error', { message: error instanceof Error ? error.message : 'Unknown error', traceId }); }
