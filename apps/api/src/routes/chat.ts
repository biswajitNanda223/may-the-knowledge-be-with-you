...
export const chatRoutes: FastifyPluginAsync = async (app) => {
  ...
  const agent = createAgentStrategy();
  ...
  app.post(
    "/",
    {
      schema: {
        tags: ["Chat"],
        summary: "Stream answer tokens and exact Neo4j evidence over SSE",
        body: {
          type: "object",
          required: ["question"],
          properties: {
        ...
        response: {
          200: {
            description: "SSE events: trace, token, complete, or error",
            type: "string",
          },
        },
      },
      ...
      try {
        ...
        await audit
          ...
          .catch((error) => {
            req.log.error(
              { err: error, traceId },
              "durable audit start failed",
            );
            if (config.AUDIT_REQUIRED) throw error;
          });
        ...
        for await (const token of agent.stream({
          ...
          if (traceAdk) adkTelemetry.chunk(traceId, token);
          send("token", { token });
        }
        if (traceAdk) adkTelemetry.complete(traceId);
        await audit.complete(traceId).catch((error) => {
          req.log.error(
            { err: error, traceId },
            "durable audit completion failed",
          );
          if (config.AUDIT_REQUIRED) throw error;
        });
        ...
        await audit
          .fail(traceId, error instanceof Error ? error.name : "UnknownError")
          .catch(() => undefined);
        send("error", {
