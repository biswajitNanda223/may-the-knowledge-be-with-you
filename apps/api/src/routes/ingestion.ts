...
export const ingestionRoutes: FastifyPluginAsync = async (app) => {
  await app.register(multipart, {
    limits: { files: 1, fileSize: config.UPLOAD_MAX_BYTES },
  });
  app.post(
    "/files",
    {
      schema: {
        tags: ["Ingestion"],
        summary: "Upload one XLSX ontology or stage one TXT source",
        consumes: ["multipart/form-data"],
      },
    },
    async (req, reply) => {
      const part = await req.file();
      if (!part) return reply.code(400).send({ message: "File required" });
      const ext = extname(part.filename).toLowerCase();
      if (![".xlsx", ".txt"].includes(ext))
        return reply
          .code(415)
          .send({ message: "Only .xlsx and .txt accepted" });
      ...
      try {
        if (ext === ".xlsx")
          return {
            ...
            counts: await importOntology(path),
          };
        return reply.code(202).send({
          status: "stored",
          source: part.filename,
          message:
            "TXT upstream parser intentionally staged for later normalized ingestion.",
        });
      } finally {
